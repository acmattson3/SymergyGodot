...
static func _map_domain(value: float, from_domain: Dictionary, to_domain: Dictionary) -> float:
	var from_lb = from_domain.get("lb", 0.0)  # Default to 0.0 if missing
	var from_ub = from_domain.get("ub", 1.0)  # Default range
	var to_lb = to_domain.get("lb", 0.0)
	var to_ub = to_domain.get("ub", 1.0)
	
	return remap(value, from_lb, from_ub, to_lb, to_ub)
